...
Purchase::Purchase(std::string id, std::string invoice, std::string date, float amout)
	{
    this->id = id;
...
bool Purchase::LoadPurchase(std::multimap<std::string, float> & ptable, const std::string  filename, bool verbose )
{
	std::ifstream infile (filename);
	
	std::list<Purchase> pod;
	...
	if(infile.is_open()){
		...
		while(getline(infile,line))
		{
			...
			Purchase p = Purchase(badge,check,duration,number);
